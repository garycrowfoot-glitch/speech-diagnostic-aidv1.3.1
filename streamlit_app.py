...
def load_speech_rules():
    ...
    try:
        ...
        data = {
            ...
            'Typical phonetic/phonological patterns': ['Difficulty producing specific speech sounds',
...
def transcribe_audio_to_text(audio_file):
    ...
    try:
        ...
        # Save uploaded file to temporary location (keep suffix if possible)
        orig_suffix = os.path.splitext(audio_file.name)[1] if hasattr(audio_file, "name") else ".wav"
        with tempfile.NamedTemporaryFile(delete=False, suffix=orig_suffix or '.wav') as tmp_file:
...
# ----------------------------
# NEW: IPA diagnostics helper
# ----------------------------

def ipa_diagnostics(ipa_text: str):
    """
    Inspect whether a string contains IPA-range Unicode.
    Returns a dict with counts and booleans.
    """
...
# ----------------------------
# UPDATED: Text → IPA (Option B fallback)
# ----------------------------
...
def text_to_ipa(text):
    ...
    try:
        ...
        # Preferred backend: Flite for eng-Latn; fallback to SimpleEpitran if needed.
        try:
        ...
        # Token-wise transliteration (safer with punctuation and mixed content)
        tokens = text.split()
        ipa_tokens = []
        for t in tokens:
            try:
                seg = epi.transliterate(t)
                ipa_tokens.append(seg if seg else t)
            except Exception:
                ipa_tokens.append(t)
...
def generate_pdf_report(analysis_results, clinician_notes=""):
    ...
    pdf.cell(0, 5, "Version 1.3 \n Copyright 2024 Gary Crowfoot", ln=True, align='C')
...
with col_right:
    st.caption("📧 [Contact](mailto:garyle.edu.au")
...
with st.sidebar:
    ...
    # NEW: Runtime diagnostic for English backend availability (Flite)
    try:
        import epitran
        try:
            epitran.Epitran('eng-Latn')  # tries Flite-backed English
            st.success("✅ eng-Latn backend (Flite) available")
        except Exception as e:
            st.warning(f"⚠️ eng-Latn backend unavailable. Fallback will be used. ({type(e).__name__})")
    except Exception:
        pass

    st.markdown("---")

    # NEW: Optional IPA Test Samples (sanity check without audio)
    st.subheader("🧪 IPA Test Samples")
    test_words = ["cat", "dog", "she", "the", "think", "nurse", "children"]
    for word in test_words:
        ipa, ok, err = text_to_ipa(word)
        st.write(f"**{word}** → {ipa if ok else err}")
...
if analyze_button:
    with st.spinner("🎧 Transcribing audio..."):
        ...
        if not success:
            # UPDATED: Never show blank error messages
            st.error(f"❌ **Error:** {error or 'Unknown IPA conversion failure.'}")
...
if st.session_state.analysis_complete and st.session_state.analysis_results:
    ...
    with tab2:
        ...
        with comp_col2:
            st.markdown("**Produced IPA (Unicode):**")
        ...
        # NEW: IPA Diagnostics block
        if results.get('produced_ipa'):
            st.markdown("---")
...
st.markdown(
    ...
    <p>Speech Diagnostic Support Tool v1.3 \n © 2024 Gary Crowfoot</p>
    ...
    <p>For research or collaboration enquiries: mailto:gary.crowfoot@newcastle.edu.augary.crowfoot@newcastle.edu.au</a></p>
    </div>
    """,
